...
describe("character_process Services", () => {
    ...
    describe("UserService", () => {
        test("character_process should successfully character process", async () => {
            ...
            const result = await userService.character_process(mockJobData); // Call character_process

            expect(result).toBe(true); // Expect the result to be true
            expect(userService.character_process).toHaveBeenCalledWith(mockJobData); // Verify it was called with the correct ID
        });
        ...
        test("character_process should handle errors", async () => {
            ...
            await expect(userService.character_process(mockJobData)).rejects.toThrow("Error deleting");
        });
    });
