...
describe("character_process User Services", () => {
    let userService;
    let jwtService;
    ...
    describe("UserService", () => {
        test("character_process should successfully character process", async () => {
            ...
            const result = await userService.character_process(mockJobData.Id_Story); // Call character_process

            expect(result).toBe(true); // Expect the result to be true
            expect(userService.character_process).toHaveBeenCalledWith(mockJobData.Id_Story); // Verify it was called with the correct ID
        });
        ...
        test("character_process should handle errors", async () => {
            ...
            await expect(userService.character_process(mockJobData.Id_Story)).rejects.toThrow("Error deleting");
        });
    });
