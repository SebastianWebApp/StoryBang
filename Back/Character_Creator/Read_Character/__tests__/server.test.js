...
describe("character_process Services", () => {
    ...
    describe("UserService", () => {
        test("character_process should successfully character process", async () => {
            ...
            const result = await userService.character_process(mockJobData.Filter); // Call character_process

            expect(result).toBe(true); // Expect the result to be true
            expect(userService.character_process).toHaveBeenCalledWith(mockJobData.Filter); // Verify it was called with the correct ID
        });
        ...
        test("character_process should handle errors", async () => {
            ...
            await expect(userService.character_process(mockJobData.Filter)).rejects.toThrow("Error deleting");
        });
    });
