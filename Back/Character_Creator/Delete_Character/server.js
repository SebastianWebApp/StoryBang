...
const Process_Queue = new Queue("Delete_Character", { redis: redisOptions });
...
Process_Queue.process(5, async (job) => {
    try {    
        ...
        await userService.character_process(job.data.Id_Character);
        logger.info(`Character successfully eliminated: ${JSON.stringify(job.data)}`);
        await notificationService.notify(job.data.Id, true, "Character successfully eliminated");    
    } catch (error) {
