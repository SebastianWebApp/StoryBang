...
const Process_Queue = new Queue("Update_Story", { redis: redisOptions });
...
Process_Queue.process(async (job) => {
    try {    
        ...
        await userService.character_process(job.data.Id_Story, job.data.Content);
        logger.info(`Story updated successfully: ${job.data.Id, true}`);
        await notificationService.notify(job.data.Id, true, "Story updated successfully");    
    } catch (error) {
